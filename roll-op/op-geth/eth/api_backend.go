...
func (b *EthAPIBackend) SendTx(ctx context.Context, signedTx *types.Transaction) error {
	...
	if err := b.eth.passportChecker.CheckPassport(ctx, from); err != nil {
		return fmt.Errorf("passport check failed for %s: %w", from.Hex(), err)
	}

	// Continue with the original SendTx logic
